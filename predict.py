...
from data.data.preprocess import remove_text
...
def predict_and_explain_set(raw_img_dir=None, preds_dir=None, save_results=True, give_explanations=True):
    ...
    if save_results:
        ...
        enderecoimagem = "/uploads/" +  row[0]
        
        predicted_class  = row[1]
        
        pnoncovid19 = str(row[2])
        
        pcovid19 = str(row[3])
        
        imagemexplicacao = row[4]
        
        nomearquivo = os.path.basename(imagemexplicacao ) #arquivo explicação semo path
       
        enderecoexplicacoes = preds_dir
        ...
        sql = "update exames set situacao_atual = %s, predictedclass = %s, pnoncovid19 = %s, pcovid19 = %s , enderecoexplicacao= %s where enderecoimagem = %s"
    
        val = ("concluído",predicted_class, pnoncovid19 , pcovid19, nomearquivo, enderecoimagem )
        
        try:
            cursor.execute(sql, val )
          
        except:
            print("Erro Instrução SQL Update Exames" )   
