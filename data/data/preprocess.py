...
def build_dataset(cfg):
    ...
    mila_covid_views_df = mila_df[mila_covid_pts_df & mila_views_cxrs_df].copy()  # Images for patients diagnosed with COVID-19
    ...
    if mode == 'binary':
        mila_covid_views_df.loc[:,'label'] = 1                                       # Mila images with COVID-19 diagnosis
...
def preprocess(cfg=None):
    ...
    if cfg is None:
        cfg = yaml.full_load(open("C:\Covid\Covid\src\config.yml", 'r'))  # Load config data
